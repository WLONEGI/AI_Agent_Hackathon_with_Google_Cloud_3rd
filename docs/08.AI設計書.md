...
class PlotGenerationAgent(BaseAIAgent):
    ...
    async def process(self, input_data: dict) -> dict:
        ...
        return {
            'story_structure': plot_result['story_structure'],  # 起承転結
            'basic_characters': plot_result['basic_characters'],  # 基本キャラ設定
            'theme': plot_result['theme'],  # メインテーマ
            'episodes': plot_result['episodes'],  # エピソード構造
            'genre': plot_result['genre'],  # ジャンル
            'page_count': plot_result['page_count']  # 想定ページ数
        }
...
#### Phase 2: ネーム生成エージェント（最重要工程）
```python
class NameGenerationAgent(BaseAIAgent):
    def __init__(self, redis_client, pubsub_client):
        super().__init__("phase2-name-generation", redis_client, pubsub_client)
        self.gemini_client = self.initialize_gemini_client()
        self.layout_analyzer = PanelLayoutAnalyzer()
        
    async def process(self, input_data: dict) -> dict:
        """
        ネーム生成処理（コマ割り・構成・ラフ漫画レイアウト・セリフ配置）
        """
        plot = input_data['plot']
        basic_characters = input_data['basic_characters']
        
        # ネーム生成（最も重要な工程）
        name_result = await self.generate_comprehensive_name(plot, basic_characters)
...
#### Phase 5: 画像生成・ペン入れエージェント（並列処理対応）
```python
class ImagePenningAgent(BaseAIAgent):
    def __init__(self, redis_client, pubsub_client):
        super().__init__("phase5-image-penning", redis_client, pubsub_client)
    ...
    async def process(self, input_data: dict) -> dict:
        """
        画像生成・ペン入れ処理（高品質画像並列生成・線画仕上げ）
        """
        rough_draft = input_data['rough_draft']  # Phase 4の下書き結果
        character_details = input_data['character_details']  # Phase 3のキャラ詳細
        name = input_data['name']  # Phase 2のネーム

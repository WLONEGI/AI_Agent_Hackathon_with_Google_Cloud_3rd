...
graph TB
    subgraph "Integrated AI Service"
        subgraph "7-Phase HITL Pipeline"
            P1[Phase1: コンセプト・世界観分析Agent - 12秒]
            P2[Phase2: キャラクター設定Agent - 18秒]
            P3[Phase3: プロット・ストーリー構成Agent - 15秒]
            P4[Phase4: ネーム生成Agent - 20秒]
            P5[Phase5: シーン画像生成Agent - 25秒]
            P6[Phase6: セリフ配置Agent - 4秒]
            P7[Phase7: 最終統合・品質調整Agent - 3秒]
        end
...
#### Phase 5: シーン画像生成エージェント（高度並列処理対応）

**実装クラス名**: `Phase5ImageAgent`（backend/app/agents/phase5_image.py）
**並列度**: 最大5並列（環境変数MAX_PARALLEL_IMAGE_GENERATION）
**性能改善**: 25秒（逐次）→ 7-10秒（並列）= **60-72%時間短縮**
...
HITL Phase Specifications:
  ...
  Phase4_NameGeneration:
    AI_Input: {phase1_concept: dict, phase2_character: dict, phase3_plot: dict}
    AI_Output: {pages: list, panels: list, camera_angles: list, scene_descriptions: list, dialogue_placement: list}
    User_Preview: "コマ割り、構図、演出指示のプレビュー"
    Feedback_Options: ["コマ割り変更", "構図調整", "演出修正", "セリフ位置変更"]
    AI_API: Gemini Pro
    Processing_Time: "20s + 最大30分フィードバック待機"
    Quality_Metrics: [layout_quality, dramatic_effect, readability]
    Note: "Phase1の世界観情報を活用してジャンル・トーンに応じた演出を決定"
    
  Phase5_SceneImageGeneration:
    AI_Input: {phase2_character: dict, phase4_name: dict, style_preferences: dict}
    AI_Output: {generated_images: list, style_consistency: dict, quality_scores: list}
    User_Preview: "生成画像ギャラリー、スタイル確認"
    Feedback_Options: ["画像再生成", "スタイル変更", "品質向上", "色調整"]
    AI_API: Imagen 4
    Processing_Time: "25s + 最大30分フィードバック待機"
    Quality_Metrics: [image_quality, style_consistency, user_satisfaction]
    
  Phase6_DialogPlacement:
    AI_Input: {phase4_name: dict, phase5_scene: dict, dialogues: list}
    AI_Output: {bubble_positions: list, font_styles: list, effect_sounds: list, reading_flow: dict}
    User_Preview: "セリフ配置、吹き出し、効果音のプレビュー"
    Feedback_Options: ["セリフ位置調整", "フォント変更", "効果音追加/削除"]
    AI_API: Gemini Pro
    Processing_Time: "4s + 最大30分フィードバック待機"
    Quality_Metrics: [readability, visual_balance, user_approval]
    
  Phase7_FinalIntegration:
    AI_Input: {all_phases_results: dict, feedback_history: list}
    AI_Output: {manga_pages: list, final_output: dict, metadata: dict}
    User_Preview: "最終ページプレビュー、全体構成確認"
    Feedback_Options: ["最終調整", "フォーマット選択", "品質設定"]
    AI_API: Gemini Pro
    Processing_Time: "3s + 最大30分フィードバック待機"
    Quality_Metrics: [overall_coherence, production_quality, user_final_satisfaction]

Total_Process_Time: "97秒 (AI処理) + 最大3.5時間 (ユーザーフィードバック: 最大30分×7フェーズ)"
User_Engagement: "各フェーズで積極的なユーザー参加、リアルタイムフィードバック統合"
Data_Flow: "Phase1の世界観情報は全フェーズで参照可能、特にPhase4で演出決定に活用"
```
...
parallel_phases = {
    "phase2_character": {
        "dependencies": ["phase1_concept_output"],
        "independent_operations": [
            "character_detail_generation",
            "visual_reference_creation", 
            "style_consistency_analysis"
        ]
    },
    "phase3_plot": {
        "dependencies": ["phase1_concept_output"],  # phase2_characterと独立
        "independent_operations": [
...
async def execute_parallel_phases_2_3(self, phase1_concept_output):
    """Phase 2とPhase 3の並列実行"""
    phase2_character_task = asyncio.create_task(
        self.execute_phase2_character(phase1_concept_output)
    )
    phase3_plot_task = asyncio.create_task(
        self.execute_phase3_plot(phase1_concept_output)
    )
    
    # 並列実行・結果統合
    phase2_character_result, phase3_plot_result = await asyncio.gather(
        phase2_character_task, phase3_plot_task
    )
    
    return self.integrate_phase2_3_results(phase2_character_result, phase3_plot_result)
```
...
class OptimizedDataFlow:
    def __init__(self):
        self.character_cache = {}  # Phase 2 結果キャッシュ
        self.story_cache = {}      # Phase 3 結果キャッシュ
        
    async def phase4_optimized_input(self, phase2_character_result, phase3_plot_result):
        """Phase 4 向け最適化データ統合"""
        return {
            "characters": phase2_character_result["detailed_characters"],
            "plot_structure": phase3_plot_result["optimized_structure"],
            "pacing_metadata": phase3_plot_result["pacing_analysis"],
            "visual_references": phase2_character_result["visual_references"],
            "integration_checksum": self.calculate_consistency_checksum(
                phase2_character_result, phase3_plot_result
            )
        }
```

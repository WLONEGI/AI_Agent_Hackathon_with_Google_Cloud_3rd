...
- [3. エンドポイント仕様](#3-エンドポイント仕様)
  ...
  - [3.2 フィードバックAPI](#32-フィードバックapi)
  - [3.3 作品管理API](#33-作品管理api)
  - [3.4 ユーザー管理API](#34-ユーザー管理api)
  - [3.5 システムAPI](#35-システムapi)
- [4. データモデル](#4-データモデル)
...
data: {"phase": 1, "duration": 90, "preview_url": "string", "next_phase": 2}
...
eventSource.addEventListener('progress', (event) => {
  ...
  console.log(`Module ${data.module}: ${data.progress}%`);
});
...
export const ERROR_CODES = {
  ...
  // バリデーションエラー
  VALID_001: 'VALID_001', // 必須項目不足
  
  // レート制限エラー
  RATE_001: 'RATE_001', // 日次生成上限到達
  
  // リソースエラー
  RES_001: 'RES_001', // プロジェクト未発見
  
  // サーバーエラー
  SRV_001: 'SRV_001', // 内部サーバーエラー
  
  // AI生成エラー
  AI_001: 'AI_001',  // AI生成サービス障害
...
#### 接続エンドポイント
```
WSS /ws/generation/{session_id}?token={jwt_token}
```
...
interface WebSocketClientMessage {
  type: 'ping' | 'auth' | 'subscribe' | 'unsubscribe'
  data: {
    token?: string        // auth時
...
interface WebSocketServerMessage {
  type: 'pong' | 'auth_success' | 'auth_required' | 
        'generation_update' | 'module_complete' | 'complete' | 'error'
  data: {
    // 統一エラーフォーマット（error時）

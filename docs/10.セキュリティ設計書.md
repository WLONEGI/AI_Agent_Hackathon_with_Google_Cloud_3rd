...
#### HITL (Human-in-the-Loop) セキュリティ設計
```python
class HITLSecurityManager:
    """
    Human-in-the-Loop フィードバックシステムのセキュリティ管理
    """
    
    def __init__(self):
        self.feedback_validator = FeedbackValidator()
        self.chat_sanitizer = ChatMessageSanitizer()
        self.version_controller = PreviewVersionAccessController()
    
    async def validate_feedback_request(self, request: dict, user_id: str) -> dict:
        """
        フィードバックリクエストのセキュリティ検証
        """
        # リクエストIDの所有権確認
        if not await self.verify_request_ownership(request['request_id'], user_id):
            raise SecurityError("リクエストへのアクセス権限がありません")
        
        # フェーズの妥当性確認
        if request['phase'] not in range(1, 8):
            raise ValidationError("無効なフェーズ番号です")
        
        # フィードバック内容のサニタイズ
        sanitized_feedback = self.sanitize_feedback_content(request['message'])
        
        return {
            'is_valid': True,
            'sanitized_message': sanitized_feedback
        }
    
    async def secure_chat_message(self, message: dict) -> dict:
        """
        チャットメッセージのセキュリティ処理
        """
        # XSS対策: HTMLエスケープ
        message['content'] = self.html_escape(message['content'])
        
        # SQLインジェクション対策: パラメータ化
        message['content'] = self.parameterize_input(message['content'])
        
        # 悪意のあるスクリプト検出
        if self.detect_malicious_patterns(message['content']):
            raise SecurityError("不正なコンテンツが検出されました")
        
        # メッセージ長制限
        if len(message['content']) > 2000:
            raise ValidationError("メッセージが長すぎます（最大2000文字）")
        
        return message
    
    def detect_malicious_patterns(self, content: str) -> bool:
        """
        悪意のあるパターンの検出
        """
        patterns = [
    ...
    async def validate_preview_access(self, version_id: str, user_id: str) -> bool:
        """
        プレビューバージョンへのアクセス権限確認
        """
        # バージョンの所有者確認
        version_owner = await self.get_version_owner(version_id)
        if version_owner != user_id:
            return False
        
        # バージョンの有効期限確認（24時間）
        creation_time = await self.get_version_creation_time(version_id)
        if datetime.now() - creation_time > timedelta(hours=24):
            return False
        
        return True
    
    async def enforce_feedback_timeout(self, request_id: str, phase: int) -> bool:
        """
        フィードバックタイムアウト（30分）の強制
        """
        start_time = await self.get_phase_start_time(request_id, phase)
        current_time = datetime.now()
        
        if current_time - start_time > timedelta(minutes=30):
            # タイムアウト後の自動処理続行
            await self.auto_continue_after_timeout(request_id, phase)
            return False
...
#### WebSocketセキュリティ
```python
class WebSocketSecurityHandler:
    """
    WebSocket通信のセキュリティ管理
    """
    ...
    async def authenticate_websocket(self, websocket, token: str) -> bool:
        """
        WebSocket接続の認証
        """
        try:
            ...
            # 接続数制限（1ユーザー最大5接続）
            if await self.count_user_connections(user_id) >= 5:
                return False
    ...
    async def validate_websocket_message(self, message: dict) -> bool:
        """
        WebSocketメッセージの検証
        """
        # メッセージタイプの検証
        valid_types = ['phase_complete', 'feedback', 'preview_request', 'chat_message']
        if message.get('type') not in valid_types:
            return False
        
        # メッセージサイズ制限（10KB）
        if len(json.dumps(message)) > 10240:
            return False
        
        # レート制限（1秒間に10メッセージまで）
        if not await self.rate_limiter.check_rate(message['user_id']):
            return False
        
        return True
    
    async def encrypt_websocket_data(self, data: dict) -> str:
        """
        WebSocket送信データの暗号化
        """
        # 重要なデータの暗号化
        if 'sensitive_data' in data:
            data['sensitive_data'] = self.encrypt_aes(data['sensitive_data'])

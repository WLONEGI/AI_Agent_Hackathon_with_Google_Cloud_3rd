...
import { InteractiveEditor } from '@/components/features/phase/InteractiveEditor';
import { VersionManager } from '@/components/features/version/VersionManager';
import { QualityAdjuster, type QualitySettings } from '@/components/features/quality/QualityAdjuster';
...
export default function HITLProcessingPage() {
  ...
  const [editingPhase, setEditingPhase] = useState<PhaseId | null>(null);
  const [showVersionManager, setShowVersionManager] = useState(false);
  const [showQualityAdjuster, setShowQualityAdjuster] = useState(false);
  ...
  // インタラクティブ編集ハンドラー
  const handleEditPhase = (phaseId: PhaseId) => {
    setEditingPhase(phaseId);
    setShowEditor(true);
    addLog(`フェーズ${phaseId}のインタラクティブ編集を開始`, 'system', phaseId);
  };

  const handleSaveEdit = (updatedData: any) => {
    if (editingPhase) {
      setPhaseData(prev => ({ ...prev, [editingPhase]: updatedData }));
      addLog(`フェーズ${editingPhase}の編集を保存しました`, 'complete', editingPhase);
      setShowEditor(false);
  ...
  const handleVersionRestore = (version: any) => {
    setPhaseData(version.data);
    addLog(`バージョン「${version.name}」を復元しました`, 'system');
  ...
  const handleQualityChange = (settings: QualitySettings) => {
    ...
    addLog(`品質レベルを${settings.level}に変更しました`, 'system');
  };
  ...
  return (
    <div className="h-screen overflow-hidden bg-[rgb(var(--bg-primary))] flex flex-col">
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button 
                onClick={() => setShowQualityAdjuster(!showQualityAdjuster)} 
                variant="ghost" 
                size="sm"
                title="品質設定"
              >
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {/* 編集ボタン */}
                        {hasData && (status === 'completed' || status === 'waiting_feedback') && (
                          <Button 
                            size="sm" 
                            variant="ghost" 
      ...
      {/* インタラクティブエディタモーダル */}
      {showEditor && editingPhase && phaseData[editingPhase] && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-[rgb(var(--bg-primary))] rounded-lg w-full max-w-6xl h-[90vh] overflow-hidden">
            <InteractiveEditor
              phaseId={editingPhase}
              data={phaseData[editingPhase]}
      ...
      {/* バージョン管理サイドパネル */}
      {showVersionManager && (
        <div className="fixed right-0 top-0 h-full w-96 bg-[rgb(var(--bg-secondary))] border-l border-[rgb(var(--border-default))] z-40 shadow-lg">
          <div className="h-full overflow-y-auto">
            <div className="p-3 border-b border-[rgb(var(--border-default))] flex items-center justify-between">
      ...
      {showQualityAdjuster && (
        <div className="fixed right-0 top-0 h-full w-96 bg-[rgb(var(--bg-secondary))] border-l border-[rgb(var(--border-default))] z-40 shadow-lg">
          <div className="h-full overflow-y-auto">
            <div className="p-3 border-b border-[rgb(var(--border-default))] flex items-center justify-between">
              <h3 className="font-semibold">品質設定</h3>
              <Button 
                size="sm" 
                variant="ghost" 
                onClick={() => setShowQualityAdjuster(false)}
              >
                ✕
              </Button>
            </div>
            <div className="p-3">
              <QualityAdjuster
                ...
                currentPhaseId={currentPhase || undefined}
              />
            </div>
          </div>
        </div>

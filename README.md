# 書けば、描ける呪文 - Spell
## Google Cloud AI Agent Hackathon Vol.3 参加作品
...
日本には多くのクリエイターが存在します。「小説家になろう」には278万人の登録ユーザーが存在し、120万作品以上が投稿されています。「カクヨム」にも100万人のユーザーが50万作品を投稿しています。これらのプラットフォームだけで、実に300万人以上が物語を紡いでいるのです。
...
| 依頼内容 | 費用相場 |
|---------|----------|
| 漫画制作（モノクロ） | 1ページあたり1〜2万円 |
| 漫画制作（カラー） | 1ページあたり8,000〜15,000円 |
| 100ページの漫画 | 100〜200万円 |
| イラスト単体（10cm×10cm） | 5,000〜10,000円 |

個人の創作者にとって、これらの金額は現実的な選択肢とは言えません。趣味で書いた小説を漫画化するために100万円以上を投資できる人は、ごく限られているでしょう。

三つ目の壁は、**既存AI画像生成ツールの限界**です。

MidjourneyやStable Diffusionは確かに革新的なツールです。Midjourneyは1,900万人のユーザーを獲得し、年間売上は3億ドル規模に達しています。しかし、これらのツールは「単発のイラスト」を生成することはできても、「ストーリーを持った漫画」を作ることはできません。

漫画形式への自動変換機能は存在せず、コマ割り、吹き出し、効果線といった漫画特有の要素を自動生成することはできません。100ページの漫画を作ろうとすれば、100ページ分の画像を個別に生成し、手動で管理する必要があります。ストーリーの流れを考慮した構成は完全に人力に頼らざるを得ず、生成後もセリフの配置やコマ間の調整など、膨大な編集作業が必要となります。

### なぜ今まで解決されていないのか

この問題が今まで解決されてこなかった理由は、技術的な困難さとコストの両面にあります。

漫画は単なる絵の集合ではありません。読者の視線を誘導し、時間の流れを表現し、感情を演出する総合芸術です。テキストから場面転換や感情の起伏を読み取り、どのシーンを大きく見せ、どこで間を取るかという演出判断を行い、キャラクターや世界観を全編通して統一し、日本独特の漫画文法を正しく適用する。これらすべてを統合的に処理するシステムの開発は、技術的に極めて困難でした。

また、仮に技術的に可能だとしても、開発には莫大な投資が必要です。特に日本の漫画市場に特化したシステムとなると、グローバル企業にとっては優先順位が低く、国内企業には技術力が不足していました。

### 私たちのアプローチ

この状況を変えるため、私たちはGoogle CloudのAI技術を最大限に活用し、まったく新しいアプローチを取りました。

単純な自動化ではなく、各段階で人間のフィードバックを受け入れる7段階のHITL（Human-in-the-Loop）処理システムを構築しました。コンセプト分析から最終統合まで、7つのフェーズに分けることで、各段階での品質を保証しながら、ユーザーの意図を確実に反映できる仕組みを実現しました。

Gemini Proによる高度なテキスト理解と自然言語処理、Imagen 4による高品質な画像生成、Cloud Runによるスケーラブルな実行環境、そして並列処理による高速化。これらのGoogle Cloud技術を組み合わせることで、専門技術なし、低コスト、高速で、誰もが自分の物語を漫画にできるサービス「Spell」が誕生しました。
...
[YouTube動画埋め込み予定 - 3〜5分のデモ動画]

実際の小説テキストから漫画が生成される様子をご覧ください。7つのフェーズが順次実行され、各段階でフィードバックを入力しながら、理想の漫画が完成していく過程を収録しています。
...
| フェーズ | 処理内容 | 使用技術 |
|---------|----------|----------|
| Phase 1: コンセプト・世界観分析 | テキストから物語のテーマ、ジャンル、世界観を自動分析し、作品の方向性を決定 | Gemini Pro |
| Phase 2: キャラクター設定 | 登場人物の性格、外見、関係性を抽出し、一貫性のあるビジュアルを生成 | Imagen 4 |
| Phase 3: プロット構成 | 起承転結の構成を自動生成し、重要シーンの判別とページ配分を最適化 | Gemini Pro |
| Phase 4: ネーム生成 | 各ページのコマ割りを自動設計し、視線誘導を考慮した配置を決定 | Gemini Pro |
| Phase 5: シーン画像生成 | 高品質な画像を並列処理で生成し、キャラクターと背景の統一感を維持 | Imagen 4 |
| Phase 6: セリフ配置 | 吹き出しの自動生成と配置、感情に応じた形状選択、効果音の追加 | Gemini Pro |
| Phase 7: 最終統合 | 全ページの統合とバランス調整を行い、PDF/WebP形式で出力 | - |

この7段階により、従来なら数ヶ月かかる作業を、わずかな時間で完了させることが可能になりました。
...
| カテゴリ | 技術 | 用途 |
|---------|------|------|
| AI/ML | Vertex AI (Gemini Pro) | テキスト理解、自然言語処理 |
| AI/ML | Imagen 4 | 高品質画像生成 |
| 実行環境 | Cloud Run | スケーラブルなコンテナ実行 |
| バックエンド | Python (FastAPI) | API開発、処理ロジック |
| フロントエンド | シンプルなWeb UI | ユーザーインターフェース |
| データベース | Cloud SQL | ユーザーデータ管理 |
| データベース | AlloyDB | ベクトルデータ保存 |
| ストレージ | Cloud Storage | 生成画像、PDFの保存 |
| メッセージング | Pub/Sub | 非同期処理、イベント駆動 |
